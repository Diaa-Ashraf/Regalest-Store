...
<head>
    ...
    <!-- Alpine.js Direct Fallback (to guarantee interactive dropdowns) -->
...
<body class="bg-[#F8F6F2] text-[#18181B] antialiased overflow-hidden">
    ...
    <div class="flex h-screen w-screen overflow-hidden">
        ...
        <!-- 1. Fixed Sidebar -->
        <aside class="w-72 flex-shrink-0 bg-white border-e border-[#EADBCC] flex flex-col h-full z-30 shadow-sm">
            ...
            <div class="h-20 flex items-center gap-3 px-6 border-b border-[#EADBCC]">
                ...
                <div class="min-w-0">
                    ...
                    <span class="text-[11px] text-[#71717A] block">لوحة الإدارة الملكية</span>
                </div>
            </div>

            <!-- Sidebar Navigation Menu -->
            <nav class="flex-1 overflow-y-auto px-4 py-4 space-y-1 admin-scrollbar">
                
                <a href="{{ route('admin.dashboard') }}" class="flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-xs font-semibold transition-all {{ request()->routeIs('admin.dashboard') ? 'bg-[#C5A059] text-white shadow-sm' : 'text-[#52525B] hover:bg-[#F8F6F2] hover:text-[#18181B]' }}">
                    <span class="text-base">📊</span>
                    <span>{{ __('الرئيسية والإحصائيات') }}</span>
                </a>

                <a href="{{ route('admin.products.index') }}" class="flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-xs font-semibold transition-all {{ request()->routeIs('admin.products.*') ? 'bg-[#C5A059] text-white shadow-sm' : 'text-[#52525B] hover:bg-[#F8F6F2] hover:text-[#18181B]' }}">
                    <span class="text-base">⌚</span>
                    <span>{{ __('المنتجات والمخزون') }}</span>
                </a>

                <a href="{{ route('admin.categories.index') }}" class="flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-xs font-semibold transition-all {{ request()->routeIs('admin.categories.*') ? 'bg-[#C5A059] text-white shadow-sm' : 'text-[#52525B] hover:bg-[#F8F6F2] hover:text-[#18181B]' }}">
                    <span class="text-base">📁</span>
                    <span>{{ __('التصنيفات والأقسام') }}</span>
                </a>

                <a href="{{ route('admin.bundles.index') }}" class="flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-xs font-semibold transition-all {{ request()->routeIs('admin.bundles.*') ? 'bg-[#C5A059] text-white shadow-sm' : 'text-[#52525B] hover:bg-[#F8F6F2] hover:text-[#18181B]' }}">
                    <span class="text-base">🎁</span>
                    <span>{{ __('العروض المجمعة (Bundles)') }}</span>
                </a>

                <a href="{{ route('admin.deals.index') }}" class="flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-xs font-semibold transition-all {{ request()->routeIs('admin.deals.*') ? 'bg-[#C5A059] text-white shadow-sm' : 'text-[#52525B] hover:bg-[#F8F6F2] hover:text-[#18181B]' }}">
                    <span class="text-base">🏷️</span>
                    <span>{{ __('التخفيضات الخاصة') }}</span>
                </a>

                <a href="{{ route('admin.orders.index') }}" class="flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-xs font-semibold transition-all {{ request()->routeIs('admin.orders.*') ? 'bg-[#C5A059] text-white shadow-sm' : 'text-[#52525B] hover:bg-[#F8F6F2] hover:text-[#18181B]' }}">
                    <span class="text-base">📦</span>
                    <span>{{ __('إدارة الطلبات') }}</span>
                </a>

                <a href="{{ route('admin.abandoned-carts.index') }}" class="flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-xs font-semibold transition-all {{ request()->routeIs('admin.abandoned-carts.*') ? 'bg-[#C5A059] text-white shadow-sm' : 'text-[#52525B] hover:bg-[#F8F6F2] hover:text-[#18181B]' }}">
                    <span class="text-base">🛒</span>
                    <span>{{ __('السلات المتروكة') }}</span>
                </a>

                <a href="{{ route('admin.analytics.index') }}" class="flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-xs font-semibold transition-all {{ request()->routeIs('admin.analytics.*') ? 'bg-[#C5A059] text-white shadow-sm' : 'text-[#52525B] hover:bg-[#F8F6F2] hover:text-[#18181B]' }}">
                    <span class="text-base">💬</span>
                    <span>{{ __('تحليلات واتساب') }}</span>
                </a>

                <a href="{{ route('admin.banners.index') }}" class="flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-xs font-semibold transition-all {{ request()->routeIs('admin.banners.*') ? 'bg-[#C5A059] text-white shadow-sm' : 'text-[#52525B] hover:bg-[#F8F6F2] hover:text-[#18181B]' }}">
                    <span class="text-base">🖼️</span>
                    <span>{{ __('البانرات والإعلانات') }}</span>
                </a>

                <div class="border-t border-[#EADBCC] my-2"></div>

                <a href="{{ route('admin.users.index') }}" class="flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-xs font-semibold transition-all {{ request()->routeIs('admin.users.*') ? 'bg-[#C5A059] text-white shadow-sm' : 'text-[#52525B] hover:bg-[#F8F6F2] hover:text-[#18181B]' }}">
                    <span class="text-base">👥</span>
                    <span>{{ __('المستخدمين والأدوار') }}</span>
                </a>

                <a href="{{ route('admin.settings.index') }}" class="flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-xs font-semibold transition-all {{ request()->routeIs('admin.settings.*') ? 'bg-[#C5A059] text-white shadow-sm' : 'text-[#52525B] hover:bg-[#F8F6F2] hover:text-[#18181B]' }}">
                    <span class="text-base">⚙️</span>
                    <span>{{ __('إعدادات المتجر') }}</span>
                </a>

                <div class="pt-3">
                    <a href="{{ route('site.home') }}" target="_blank" class="flex items-center justify-center gap-2 w-full py-2.5 px-3 rounded-xl bg-[#F8F6F2] hover:bg-[#EADBCC] text-[#18181B] text-xs font-bold border border-[#EADBCC] transition-all">
                        <span>👁️</span>
            ...
            <div class="p-4 border-t border-[#EADBCC] bg-[#F8F6F2]/60">
                <div class="flex items-center gap-3">
                    <div class="w-8 h-8 rounded-full bg-[#18181B] text-[#C5A059] flex items-center justify-center font-bold text-xs flex-shrink-0">
                        {{ substr(auth()->user()->name ?? 'A', 0, 1) }}
                    </div>
                    <div class="min-w-0 flex-1">
                        <p class="text-xs font-bold text-[#18181B] truncate">{{ auth()->user()->name ?? 'Admin' }}</p>
                        <span class="text-[10px] text-[#C5A059] block truncate">المدير العام</span>
                    </div>
                    <form action="{{ route('logout') }}" method="POST">
                        @csrf
                        <button type="submit" class="text-red-500 hover:text-red-700 p-1" title="تسجيل الخروج">
                            🚪
                        </button>
                    </form>
                </div>
        ...
        <div class="flex-1 flex flex-col min-w-0 h-full overflow-y-auto">
            ...
            <!-- Sticky Top Header -->
            <header class="h-20 px-8 bg-white/95 backdrop-blur-md border-b border-[#EADBCC] flex items-center justify-between sticky top-0 z-20 flex-shrink-0">
                ...
                <div class="flex items-center gap-4 flex-1 max-w-md">
                    <div class="relative w-full">
                        <input type="text" 
                               ...
                               class="w-full ps-10 pe-4 py-2 text-xs bg-[#F8F6F2] border border-[#EADBCC] rounded-xl text-[#18181B] placeholder-[#71717A] focus:outline-none focus:border-[#C5A059]">
                ...
                <div class="flex items-center gap-4">
                    <div class="relative" x-data="{ open: false }">
                        <button @click="open = !open" type="button" class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-[#EADBCC] bg-[#F8F6F2] text-xs font-semibold text-[#18181B]">
    ...
    <div id="deleteModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#18181B]/60 backdrop-blur-sm opacity-0 pointer-events-none transition-all duration-200">
        <div class="relative w-full max-w-md bg-white border border-[#EADBCC] rounded-2xl shadow-2xl p-6 sm:p-7 text-center transform scale-95 transition-all duration-200" id="deleteModalCard">
            ...
            <div class="flex items-center justify-center gap-3">
                <button type="button" onclick="closeDeleteModal()" class="px-5 py-2.5 rounded-xl border border-[#EADBCC] bg-[#F8F6F2] hover:bg-[#EDE8DC] text-[#18181B] text-xs sm:text-sm font-semibold transition-colors flex-1">
                    {{ __('إلغاء') }}
                </button>
                <button type="button" id="deleteModalConfirmBtn" class="px-5 py-2.5 rounded-xl bg-rose-600 hover:bg-rose-700 text-white text-xs sm:text-sm font-semibold shadow-sm transition-all flex-1">
                    {{ __('نعم، احذف') }}
                </button>
            </div>
        </div>
    ...
    <script>
        ...
        document.addEventListener('submit', function(e) {
            const form = e.target;
            const isDelete = form.querySelector('input[name="_method"][value="DELETE"]') || form.getAttribute('data-confirm');
            
            // If already confirmed by modal or is not delete, let it pass
