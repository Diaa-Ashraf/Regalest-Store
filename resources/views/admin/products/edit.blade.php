...
<div class="max-w-5xl mx-auto space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <h2 class="text-2xl font-bold text-[#18181B] tracking-tight">{{ __('تعديل المنتج') }}</h2>
            <p class="text-sm text-[#71717A] mt-0.5">{{ __('تحديث مواصفات وبيانات المنتج') }}: <span class="font-semibold text-[#18181B]">{{ $product->name }}</span></p>
        </div>
        ...
        <form action="{{ route('products.update', $product->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" id="name_ar" name="ar[name]" 
                           ...
                           value="{{ old('ar.name', $product->translate('ar')?->name ?? $product->name) }}" required>
                    @error('ar.name')
                        <p class="text-rose-500 text-xs mt-1.5">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="text" id="name_en" name="en[name]" 
                           ...
                           value="{{ old('en.name', $product->translate('en')?->name) }}">
                    @error('en.name')
                        <p class="text-rose-500 text-xs mt-1.5">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <textarea id="desc_ar" name="ar[description]" rows="3"
                           class="w-full px-4 py-2.5 rounded-xl border border-[#EADBCC] bg-[#F8F6F2]/40 text-[#18181B] focus:border-[#C5A059] focus:ring-1 focus:ring-[#C5A059] focus:outline-none text-sm transition-all @error('ar.description') border-rose-500 @enderror">{{ old('ar.description', $product->translate('ar')?->description ?? $product->description) }}</textarea>
                ...
                <div>
                    ...
                    <textarea id="desc_en" name="en[description]" rows="3"
                           class="w-full px-4 py-2.5 rounded-xl border border-[#EADBCC] bg-[#F8F6F2]/40 text-[#18181B] focus:border-[#C5A059] focus:ring-1 focus:ring-[#C5A059] focus:outline-none text-sm transition-all @error('en.description') border-rose-500 @enderror">{{ old('en.description', $product->translate('en')?->description) }}</textarea>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    @php
                        $arKeywords = $product->translate('ar')?->keywords ?? $product->keywords;
                        $arKeywordsVal = is_array($arKeywords) ? implode(', ', $arKeywords) : $arKeywords;
                    @endphp
                    <input type="text" id="keywords_ar" name="ar[keywords]" 
                           ...
                           value="{{ old('ar.keywords', $arKeywordsVal) }}">
                    @error('ar.keywords')
                        <p class="text-rose-500 text-xs mt-1.5">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    @php
                        $enKeywords = $product->translate('en')?->keywords;
                        $enKeywordsVal = is_array($enKeywords) ? implode(', ', $enKeywords) : $enKeywords;
                    @endphp
                    <input type="text" id="keywords_en" name="en[keywords]" 
                           ...
                           value="{{ old('en.keywords', $enKeywordsVal) }}">
                    @error('en.keywords')
                        <p class="text-rose-500 text-xs mt-1.5">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    @if($product->image_url)
                        <div class="mb-2 flex items-center gap-3">
                            <img src="{{ $product->image_url }}" class="w-14 h-14 rounded-xl object-cover border border-[#EADBCC]" alt="{{ $product->name }}">
                            <span class="text-xs text-[#71717A]">الصورة الحالية</span>
                        </div>
                    @endif
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 pt-2">
                ...
                <div>
                    ...
                    <input type="number" id="stock_quantity" name="stock_quantity" 
                           ...
                           value="{{ old('stock_quantity', $product->stock_quantity ?? $product->quantity) }}" required>
            ...
            <div class="p-4 rounded-xl bg-[#F8F6F2]/60 border border-[#EADBCC] flex flex-wrap items-center gap-8">
                <label class="inline-flex items-center gap-3 cursor-pointer">
                    <input type="checkbox" name="featured" value="1" class="w-4 h-4 rounded text-[#C5A059] focus:ring-[#C5A059] border-[#EADBCC]" {{ $product->featured ? 'checked' : '' }}>
                    <span class="text-sm font-semibold text-[#18181B]">⭐ تمييز في الصفحة الرئيسية (Featured)</span>
                </label>

                <label class="inline-flex items-center gap-3 cursor-pointer">
                    <input type="checkbox" name="is_active" value="1" class="w-4 h-4 rounded text-emerald-600 focus:ring-emerald-500 border-[#EADBCC]" {{ $product->is_active ? 'checked' : '' }}>
