...
@section('title', __('تعديل التخفيض'))

@section('content')
<div class="max-w-4xl mx-auto space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-2xl font-bold text-[#18181B]">{{ __('تعديل التخفيض') }}: {{ $deal->product->name ?? '' }}</h2>
            <p class="text-sm text-[#71717A] mt-0.5">{{ __('تحديث بيانات ونسبة التخفيض وتاريخ الانتهاء') }}</p>
        </div>
        ...
        <form action="{{ route('deals.update', $deal->id) }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <div class="relative">
                        <input type="number" step="0.01" name="deal_price" id="deal_price" 
                               ...
                               value="{{ old('deal_price', $deal->deal_price) }}" required>
            ...
            <div>
                ...
                <input type="text" name="badge_text" id="badge_text" 
                       ...
                       value="{{ old('badge_text', $deal->badge_text) }}" 
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
                <div>
                    ...
                    <input type="number" name="sort_order" id="sort_order" 
                           class="w-full px-4 py-2.5 rounded-xl border border-[#EADBCC] bg-[#F8F6F2]/40 text-[#18181B] focus:border-[#C5A059] focus:ring-1 focus:ring-[#C5A059] focus:outline-none text-sm transition-all" 
                           value="{{ old('sort_order', $deal->sort_order) }}" min="0">
                </div>

                <div class="p-4 rounded-xl bg-[#F8F6F2]/60 border border-[#EADBCC] mt-6">
                    <label class="inline-flex items-center gap-3 cursor-pointer">
                        <input type="checkbox" name="is_active" value="1" class="w-4 h-4 rounded text-emerald-600 focus:ring-emerald-500 border-[#EADBCC]" {{ old('is_active', $deal->is_active) ? 'checked' : '' }}>
                        <span class="text-sm font-semibold text-[#18181B]">تفعيل العرض بالمتجر</span>
                    </label>
                </div>
            </div>
