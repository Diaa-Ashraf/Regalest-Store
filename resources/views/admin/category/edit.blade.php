...
<div class="max-w-4xl mx-auto space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-2xl font-bold text-[#18181B]">{{ __('تعديل التصنيف') }}</h2>
            <p class="text-sm text-[#71717A] mt-0.5">{{ __('تعديل تفاصيل الفئة') }}: <span class="font-semibold text-[#18181B]">{{ $category->name }}</span></p>
        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="text" id="name_{{ $locale }}" name="{{ $locale }}[name]" 
                                       ...
                                       value="{{ old($locale.'.name', $category->translate($locale)?->name ?? $category->name) }}" required>
                            </div>
                            <div>
                                ...
                                <input type="text" id="desc_{{ $locale }}" name="{{ $locale }}[description]" 
                                       class="w-full px-3.5 py-2 rounded-xl border border-[#EADBCC] bg-white text-[#18181B] focus:border-[#C5A059] focus:ring-1 focus:ring-[#C5A059] focus:outline-none text-sm transition-all"
                                       value="{{ old($locale.'.description', $category->translate($locale)?->description ?? $category->description) }}">
                            </div>
                        </div>
                    </div>
                @endforeach
                ...
                <div>
                    ...
                    @if($category->image_url)
                        <div class="mb-2 flex items-center gap-3">
                            <img src="{{ $category->image_url }}" class="w-14 h-14 rounded-xl object-cover border border-[#EADBCC]" alt="{{ $category->name }}">
