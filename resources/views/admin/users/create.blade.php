...
@section('title', __('إضافة مشرف جديد'))

@section('content')
<div class="max-w-4xl mx-auto space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-2xl font-bold text-[#18181B]">{{ __('إضافة مشرف جديد') }}</h2>
            <p class="text-sm text-[#71717A] mt-0.5">{{ __('إنشاء حساب وصول جديد للوحة تحكم الإدارة الملكية') }}</p>
        </div>
    ...
    <div class="bg-white border border-[#EADBCC] rounded-2xl p-6 sm:p-8 shadow-sm">
        <form action="{{ route('users.store') }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" id="name" name="name" 
                           ...
                           value="{{ old('name') }}" required placeholder="مثال: أحمد محمد">
                    @error('name') 
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label for="password" class="block text-sm font-semibold text-[#18181B] mb-2">{{ __('كلمة المرور') }} <span class="text-rose-500">*</span></label>
                ...
                <div>
                    ...
                    <select id="role" name="role" class="w-full px-4 py-2.5 rounded-xl border border-[#EADBCC] bg-[#F8F6F2]/40 text-[#18181B] focus:border-[#C5A059] focus:ring-1 focus:ring-[#C5A059] focus:outline-none text-sm transition-all @error('role') border-rose-500 @enderror" required>
